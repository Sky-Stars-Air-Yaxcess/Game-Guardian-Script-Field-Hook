...
-- Better download the update script also safe from my GitHub: Sky-Stars-Air-Yaxcess 

local function safeUpdate()
    local localVer = 4
    local changelog = gg.alert("New Update Script Install?", "Yes", "No")
end

local function safeCodeLoader(code)
    local sandbox = {
        gg = gg,
        pairs = pairs,
        ipairs = ipairs,
...
function mainMenu()
   firstMenu = gg.multiChoice({ "Cheat 1", "Cheat 2", "Sub Menu", "❌EXIT❌" }, nil, "Fields Find_Hook🎣")
...
function subcheat_2()
   valueFromClass("PlayerController", "0x28", false, true, gg.TYPE_FLOAT, {'ChildClass', "0x70"})
   gg.getResult(10000)
...
--[[ 🎣 These function help in error log ]] --
      ...
      if SearchTypeSelection == 1 then
         Get_user_input = gg.prompt(
            ...
            SearchType[SearchTypeSelection]['topic'] .. "\n\n " .. SearchType[SearchTypeSelection]['name'] .. " : ",
               SearchType[SearchTypeSelection]['offset'] .. " : ", "Try Harder Find(decreases accuracy)", "32 Bit",
...
--[[ 🎣 It will create an UI button and loop until user clicks the UI. If clicked calls start() ]] --
function UI()
   gg.showUiButton()
   while true do
